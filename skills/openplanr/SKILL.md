...
├── Claude Code
│   │
│   └── Is the openplanr-pipeline plugin installed?
│       │
...
├── Cursor
│   │
│   └── Are pipeline rules installed (`.cursor/rules/openplanr-pipeline.mdc`)?
│       │
...
Full compatibility matrix: see `openplanr-pipeline/docs/compatibility-matrix.md`.
...
`.cursor/rules/openplanr-pipeline.mdc` (generated by
`planr rules generate --target cursor --scope pipeline`).

The Cursor rules tell Composer how to orchestrate the pipeline:

- User says "plan {feature}" → `openplanr-pipeline-plan.mdc` activates
- User says "ship {feature}" → `openplanr-pipeline-ship.mdc` activates
